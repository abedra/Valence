...
bindVars envRef bindings = readIORef envRef >>= extendedEnv bindings >>= newIORef
         where extendedEnv bindings env = liftM (++ env) (mapM addBinding bindings)
...
runRepl :: IO ()
runRepl = nullEnv >>= until_ (== "quit") (readPrompt "Valence>> ") . evalAndPrint
...
eval env (List [Atom "set1", Atom var, form]) =
